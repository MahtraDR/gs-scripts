...
# In production these are defined inside the DynMap module in map.lic.
# We redefine them here to test in isolation.
...
module DynMap
  ...
  module LayoutEngine
    ...
    def self.direction_offset(move_cmd)
      ...
      return DIRECTION_OFFSETS[d] if DIRECTION_OFFSETS[d]
      nil
    end
...
RSpec.describe DynMap::ZoneResolver do
  before(:each) do
    DynMap::ZoneResolver.clear_cache
    MockMapList.clear
  end
  ...
  describe '.has_location?' do
    it 'returns true for a non-empty string location' do
      ...
      expect(DynMap::ZoneResolver.has_location?(room)).to be true
    end

    it 'returns false for nil location' do
      room = MockRoom.new(id: 1, location: nil)
      expect(DynMap::ZoneResolver.has_location?(room)).to be false
    end
  ...
  describe '.resolve' do
    it 'returns nil for nil room' do
      expect(DynMap::ZoneResolver.resolve(nil)).to be_nil
    end
    ...
    it 'uses genie_zone as highest priority' do
      ...
      result = DynMap::ZoneResolver.resolve(r1)
    ...
    it 'caches results for same zone key' do
      ...
      result1 = DynMap::ZoneResolver.resolve(r1)
      result2 = DynMap::ZoneResolver.resolve(r1)
  ...
  describe '.bfs_zone' do
    it 'collects connected rooms without zone assignments' do
      ...
      result = DynMap::ZoneResolver.bfs_zone(r0)
    ...
    it 'stops at rooms with image (zone boundary)' do
      ...
      result = DynMap::ZoneResolver.bfs_zone(r0)
      expect(result.map(&:id)).to eq([0]) # r1 has image, stops there
    end
    ...
    it 'respects max_depth' do
      ...
      result = DynMap::ZoneResolver.bfs_zone(r0, max_depth: 1)
      expect(result.map(&:id)).to contain_exactly(0, 1)
    end
  end
end

RSpec.describe DynMap::LayoutEngine do
  describe '.direction_offset' do
    it 'returns correct offset for cardinal directions' do
      expect(DynMap::LayoutEngine.direction_offset('north')).to eq([0, -1])
      expect(DynMap::LayoutEngine.direction_offset('south')).to eq([0, 1])
      expect(DynMap::LayoutEngine.direction_offset('east')).to eq([1, 0])
      expect(DynMap::LayoutEngine.direction_offset('west')).to eq([-1, 0])
    end

    it 'returns correct offset for abbreviated directions' do
      expect(DynMap::LayoutEngine.direction_offset('n')).to eq([0, -1])
      expect(DynMap::LayoutEngine.direction_offset('se')).to eq([1, 1])
      expect(DynMap::LayoutEngine.direction_offset('nw')).to eq([-1, -1])
    end

    it 'returns correct offset for vertical directions' do
      expect(DynMap::LayoutEngine.direction_offset('up')).to eq([0, -1])
      expect(DynMap::LayoutEngine.direction_offset('down')).to eq([0, 1])
    end

    it 'returns nil for non-directional moves' do
      expect(DynMap::LayoutEngine.direction_offset('go door')).to be_nil
      expect(DynMap::LayoutEngine.direction_offset('climb ladder')).to be_nil
    end

    it 'returns nil for StringProc commands' do
      expect(DynMap::LayoutEngine.direction_offset(';e fput "go door"')).to be_nil
    end
  ...
  describe '.point_on_segment?' do
    it 'detects point on horizontal segment' do
      expect(DynMap::LayoutEngine.point_on_segment?(20, 0, 0, 0, 40, 0)).to be true
    end

    it 'detects point on vertical segment' do
      expect(DynMap::LayoutEngine.point_on_segment?(0, 20, 0, 0, 0, 40)).to be true
    end

    it 'detects point on diagonal segment' do
      expect(DynMap::LayoutEngine.point_on_segment?(20, 20, 0, 0, 40, 40)).to be true
    end

    it 'rejects point not on segment' do
      expect(DynMap::LayoutEngine.point_on_segment?(20, 10, 0, 0, 40, 0)).to be false
    end

    it 'rejects point on the line but outside segment' do
      expect(DynMap::LayoutEngine.point_on_segment?(60, 0, 0, 0, 40, 0)).to be false
    end

    it 'rejects zero-length segments' do
      expect(DynMap::LayoutEngine.point_on_segment?(0, 0, 0, 0, 0, 0)).to be false
    end

    it 'treats endpoints as on the segment' do
      expect(DynMap::LayoutEngine.point_on_segment?(0, 0, 0, 0, 40, 0)).to be true
      expect(DynMap::LayoutEngine.point_on_segment?(40, 0, 0, 0, 40, 0)).to be true
    end
  end
  ...
  describe '.layout' do
    it 'uses Genie positions when all rooms have genie_pos (Mode A)' do
      ...
      positions = DynMap::LayoutEngine.layout([r1, r2], r1)
    ...
    it 'falls back to auto-layout when not all rooms have genie_pos' do
      ...
      positions = DynMap::LayoutEngine.layout([r1, r2], r1)
      expect(positions[0]).to eq({ x: 0, y: 0, z: 0 }) # seed room at origin
      expect(positions[1][:y]).to be < positions[0][:y] # north means lower y
    end

    it 'places seed room at origin' do
      r1 = MockRoom.new(id: 0, wayto: {})
      positions = DynMap::LayoutEngine.layout([r1], r1)
    ...
    it 'places rooms in correct relative positions' do
      ...
      positions = DynMap::LayoutEngine.layout([r0, r1, r2], r0)

      # North room should be above (lower y)
      expect(positions[1][:y]).to be < positions[0][:y]
      expect(positions[1][:x]).to eq(positions[0][:x])

      # East room should be to the right (higher x)
    ...
    it 'handles non-directional moves' do
      ...
      positions = DynMap::LayoutEngine.layout([r0, r1], r0)
    ...
    it 'assigns positions to all rooms in the zone' do
      ...
      positions = DynMap::LayoutEngine.layout([r0, r1, r2, r3], r0)
    ...
    it 'handles a linear corridor' do
      ...
      # Should be a horizontal line going east
  ...
  describe '.on_connection_line?' do
    it 'detects a point on a connection line' do
      ...
      expect(DynMap::LayoutEngine.on_connection_line?(20, 0, positions, room_index)).to be true
    end
    ...
    it 'does not flag endpoints as on the line' do
      ...
      expect(DynMap::LayoutEngine.on_connection_line?(0, 0, positions, room_index)).to be false
    end
    ...
    it 'skips lines to neighbor rooms when skip_neighbors_of is provided' do
      ...
      # Room 1 is between 0 and 2, but is a neighbor of both — should be skipped
      expect(DynMap::LayoutEngine.on_connection_line?(20, 0, positions, room_index, skip_neighbors_of: 1)).to be false
    end
  end
end
